-- ==========================================================================
-- Quant Desk P&L Engine — Supabase PostgreSQL Schema
-- Run this entire script in: Supabase Dashboard → SQL Editor → New query → Run
--
-- Tables created:
--   1. daily_summaries      (primary reporting table, one row per trading day)
--   2. strategy_runs        (one row per strategy executed per day)
--   3. trade_executions     (one row per leg/execution, linked to strategy_run)
--   4. charges_breakdown    (one row per strategy_run charge record)
--   5. equity_curve         (computed daily cumulative curve, one row per day)
--
-- Notes:
--   - All monetary values are in INR (Indian Rupees).
--   - report_date uses DATE type (YYYY-MM-DD).
--   - JSONB columns store structured data (segment_breakdown, legs_greeks, etc.)
--   - Row Level Security (RLS) is DISABLED by default for service_role access.
--     Enable and configure policies if you expose this via anon key.
...
COMMENT ON TABLE public.daily_summaries IS
    'One row per trading day. Primary executive KPI table for founder dashboard.';
...
CREATE TABLE IF NOT EXISTS public.strategy_runs (
    ...
    deployment_status           TEXT        NOT NULL DEFAULT 'EXITED',   -- LIVE_AUTO | EXITED | PARTIAL | OTHER
    ...
    underlying_segment          TEXT,                                     -- NIFTY | BANKNIFTY | SENSEX | FINNIFTY | MIDCPNIFTY
    entry_timestamp_ist         TIMESTAMPTZ,
    exit_timestamp_ist          TIMESTAMPTZ,
    legs_greeks                 JSONB,                                    -- [{delta, theta, vega, iv, ltp, ...}]
    created_at                  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at                  TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_strategy_runs_report_date
    ON public.strategy_runs(report_date);

CREATE INDEX IF NOT EXISTS idx_strategy_runs_segment
    ON public.strategy_runs(underlying_segment);

COMMENT ON TABLE public.strategy_runs IS
    'One row per Tradetron strategy card per day. FK to daily_summaries.';
...
CREATE TABLE IF NOT EXISTS public.trade_executions (
    ...
    vendor_symbol       TEXT        NOT NULL,     -- e.g. BANKNIFTY2692552000CE
    underlying          TEXT,                     -- BANKNIFTY
    segment             TEXT,                     -- BANKNIFTY | NIFTY | SENSEX ...
    ...
    matched_pair_id     UUID,                     -- links entry+exit legs
    extra_metadata      JSONB,
    created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_trade_executions_report_date
    ON public.trade_executions(report_date);

CREATE INDEX IF NOT EXISTS idx_trade_executions_strategy_run_uuid
    ON public.trade_executions(strategy_run_uuid);

CREATE INDEX IF NOT EXISTS idx_trade_executions_underlying
    ON public.trade_executions(underlying);

COMMENT ON TABLE public.trade_executions IS
    'One row per executed leg from Zerodha Kite Positions.';
...
CREATE TABLE IF NOT EXISTS public.charges_breakdown (
    ...
    report_date         DATE        NOT NULL,
    charge_source       TEXT        NOT NULL DEFAULT 'FORMULA_COMPUTED',  -- REALIZED_VIRTUAL_CONTRACT_NOTE | FORMULA_COMPUTED
...
CREATE INDEX IF NOT EXISTS idx_charges_breakdown_report_date
    ON public.charges_breakdown(report_date);

CREATE INDEX IF NOT EXISTS idx_charges_breakdown_strategy_run_uuid
    ON public.charges_breakdown(strategy_run_uuid);

COMMENT ON TABLE public.charges_breakdown IS
    'Line-item Zerodha / statutory charges per strategy run per day.';
...
CREATE TABLE IF NOT EXISTS public.equity_curve (
    ...
    running_win_rate_30d    NUMERIC(10,4),                        -- rolling 30-day win %
    created_at              TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at              TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

COMMENT ON TABLE public.equity_curve IS
    'Computed daily equity curve. Rebuilt by pipeline stage 9 after each trading day.';

-- --------------------------------------------------------------------------
-- Utility: auto-update updated_at on all tables
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
-- Useful views for the founder dashboard
-- --------------------------------------------------------------------------
...
SELECT
    ...
    ROUND(
        SUM(win_count)::NUMERIC /
        NULLIF(SUM(win_count) + SUM(loss_count), 0) * 100, 2
    )                                     AS win_rate_pct,
...
-- If you use the anon key from the browser, enable RLS and add policies.
-- --------------------------------------------------------------------------
...
-- Allow service_role full access (bypasses RLS automatically in Supabase)
-- Allow authenticated users read-only access (optional — remove if not needed)
CREATE POLICY IF NOT EXISTS "authenticated_read_daily_summaries"
...
CREATE POLICY IF NOT EXISTS "authenticated_read_strategy_runs"
    ON public.strategy_runs FOR SELECT TO authenticated USING (true);

CREATE POLICY IF NOT EXISTS "authenticated_read_trade_executions"
    ON public.trade_executions FOR SELECT TO authenticated USING (true);

CREATE POLICY IF NOT EXISTS "authenticated_read_charges"
    ON public.charges_breakdown FOR SELECT TO authenticated USING (true);

CREATE POLICY IF NOT EXISTS "authenticated_read_equity_curve"
    ON public.equity_curve FOR SELECT TO authenticated USING (true);
