...
- Updated `templates/daily_report.html` as a matching, self-contained design scaffold. The live renderer still constructs HTML directly inside `src/report_engine.py`; wiring the template into that renderer remains a separate cleanup, so do not treat the scaffold as the active runtime template.
